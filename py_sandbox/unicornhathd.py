...
Fake out library to capture how data is built for:
...
def show():
    ...
    if _addressing_enabled:
        ...
        # logging.error('_buf: ')
        # logging.error(_buf)
        # logging.error('_rotation: ')
        # logging.error(_rotation)
        # logging.error('window: ')
        # logging.error(window)
        
        # logging.error('reshapeXbrightness::   ')
        # logging.error(reshapeXbrightness)
        # logging.error('rxbAsUnit8:: ')
        # logging.error(rxbAsUnit8)
        # logging.error('rxbAsUnit8.tolist() ')
        # logging.error(rxbAsUnit8.tolist())
        _spi.xfer2(_buf, _rotation, rotated, [_SOF] + (numpy.rot90(_buf, _rotation).reshape(768) * _brightness).astype(numpy.uint8).tolist())
...
class FakeSpi:
    ...
    def xfer2(self, buff, rotation, rotated, someHugeBullshitThing):
